...
const TESTS = JSON.parse("[{\"input\":{\"nums1\":[1,3],\"nums2\":[2]},\"expected\":2.0,\"visible\":true},{\"input\":{\"nums1\":[1,2],\"nums2\":[3,4]},\"expected\":2.5,\"visible\":true},{\"input\":{\"nums1\":[],\"nums2\":[1]},\"expected\":1.0,\"visible\":false},{\"input\":{\"nums1\":[2],\"nums2\":[]},\"expected\":2.0,\"visible\":false},{\"input\":{\"nums1\":[1,2,3],\"nums2\":[4,5,6]},\"expected\":3.5,\"visible\":false},{\"input\":{\"nums1\":[1],\"nums2\":[1]},\"expected\":1.0,\"visible\":false},{\"input\":{\"nums1\":[1,3,5,7],\"nums2\":[2,4,6,8]},\"expected\":4.5,\"visible\":false}]");
...
function run(solutionPath, testsPath, challengePath) {
    ...
    const fn = resolveFunction(solution, ["findMedianSortedArrays"]);
    const caseInput = test.input;
    let args = [];
    if (isObject(caseInput)) {
        args = ["nums1", "nums2"].map((key) => caseInput[key]);
    } else if (Array.isArray(caseInput) && ["nums1", "nums2"].length > 1) {
        args.push(caseInput);
        for (let i = 1; i < ["nums1", "nums2"].length; i += 1) {
            const key = ["nums1", "nums2"][i];
        ...
        args = ["nums1", "nums2"].map((key) => test[key]);
    }
    gotValue = fn(...args);
