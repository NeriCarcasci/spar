...
def run(solution_path, tests_path, challenge_path):
    ...
    for idx, case in enumerate(TESTS, start=1):
        ...
        fn = resolve_callable(module, ['find_median_sorted_arrays'])
        case_input = case.get("input")
        args = []
        if isinstance(case_input, dict):
            for key in ['nums1', 'nums2']:
                args.append(case_input.get(key))
        elif isinstance(case_input, list) and len(['nums1', 'nums2']) > 1:
            args.append(case_input)
            for idx in range(1, len(['nums1', 'nums2'])):
                key = ['nums1', 'nums2'][idx]
                if key in case:
