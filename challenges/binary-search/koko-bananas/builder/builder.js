...
const TESTS = JSON.parse("[{\"input\":{\"piles\":[3,6,7,11],\"h\":8},\"expected\":4,\"visible\":true},{\"input\":{\"piles\":[30,11,23,4,20],\"h\":5},\"expected\":30,\"visible\":true},{\"input\":{\"piles\":[1],\"h\":1},\"expected\":1,\"visible\":false},{\"input\":{\"piles\":[1000000000],\"h\":2},\"expected\":500000000,\"visible\":false},{\"input\":{\"piles\":[30,11,23,4,20],\"h\":6},\"expected\":23,\"visible\":false},{\"input\":{\"piles\":[1,1,1,1],\"h\":4},\"expected\":1,\"visible\":false},{\"input\":{\"piles\":[312884470],\"h\":968709470},\"expected\":1,\"visible\":false}]");
...
function run(solutionPath, testsPath, challengePath) {
    ...
    const fn = resolveFunction(solution, ["minEatingSpeed"]);
    const caseInput = test.input;
    let args = [];
    if (isObject(caseInput)) {
        args = ["piles", "h"].map((key) => caseInput[key]);
    } else if (Array.isArray(caseInput) && ["piles", "h"].length > 1) {
        args.push(caseInput);
        for (let i = 1; i < ["piles", "h"].length; i += 1) {
            const key = ["piles", "h"][i];
