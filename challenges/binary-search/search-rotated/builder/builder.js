...
const TESTS = JSON.parse("[{\"input\":{\"nums\":[4,5,6,7,0,1,2],\"target\":0},\"expected\":4,\"visible\":true},{\"input\":{\"nums\":[4,5,6,7,0,1,2],\"target\":3},\"expected\":-1,\"visible\":true},{\"input\":{\"nums\":[1],\"target\":1},\"expected\":0,\"visible\":false},{\"input\":{\"nums\":[1],\"target\":0},\"expected\":-1,\"visible\":false},{\"input\":{\"nums\":[3,1],\"target\":1},\"expected\":1,\"visible\":false},{\"input\":{\"nums\":[1,2,3,4,5],\"target\":3},\"expected\":2,\"visible\":false},{\"input\":{\"nums\":[5,1,2,3,4],\"target\":5},\"expected\":0,\"visible\":false}]");
...
function run(solutionPath, testsPath, challengePath) {
    ...
    const fn = resolveFunction(solution, ["searchRotated"]);
    const caseInput = test.input;
    let args = [];
    if (isObject(caseInput)) {
        args = ["nums", "target"].map((key) => caseInput[key]);
    } else if (Array.isArray(caseInput) && ["nums", "target"].length > 1) {
        args.push(caseInput);
        for (let i = 1; i < ["nums", "target"].length; i += 1) {
            const key = ["nums", "target"][i];
        ...
        args = ["nums", "target"].map((key) => test[key]);
    }
    gotValue = fn(...args);
