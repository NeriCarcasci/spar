...
def run(solution_path, tests_path, challenge_path):
    ...
    for idx, case in enumerate(TESTS, start=1):
        ...
        fn = resolve_callable(module, ['build_tree'])
        node_cls = resolve_class(module, "TreeNode", tree_node_fallback)
        inp = case.get("input", {})
        args = []
        for key in ['preorder', 'inorder']:
            value = inp.get(key)
            if key in ("root", "subRoot", "p", "q"):
                value = array_to_tree(value, node_cls)
            args.append(value)
        out = fn(*args)
        if 'trees/build-tree-preorder' in ("trees/build-tree-preorder", "trees/invert-binary-tree"):
