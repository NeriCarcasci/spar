...
int main(int argc, char** argv) {
    ...
    std::vector<Value> tests = {
        Value::object({{"input", Value::object({{"preorder", Value::array({Value(3.0), Value(9.0), Value(20.0), Value(15.0), Value(7.0)})}, {"inorder", Value::array({Value(9.0), Value(3.0), Value(15.0), Value(20.0), Value(7.0)})}})}, {"expected", Value::array({Value(3.0), Value(9.0), Value(20.0), Value(), Value(), Value(15.0), Value(7.0)})}, {"visible", Value(true)}}),
        Value::object({{"input", Value::object({{"preorder", Value::array({Value(1.0)})}, {"inorder", Value::array({Value(1.0)})}})}, {"expected", Value::array({Value(1.0)})}, {"visible", Value(true)}}),
        Value::object({{"input", Value::object({{"preorder", Value::array({Value(-1.0)})}, {"inorder", Value::array({Value(-1.0)})}})}, {"expected", Value::array({Value(-1.0)})}, {"visible", Value(false)}}),
        Value::object({{"input", Value::object({{"preorder", Value::array({Value(1.0), Value(2.0)})}, {"inorder", Value::array({Value(2.0), Value(1.0)})}})}, {"expected", Value::array({Value(1.0), Value(2.0)})}, {"visible", Value(false)}}),
        Value::object({{"input", Value::object({{"preorder", Value::array({Value(1.0), Value(2.0), Value(3.0)})}, {"inorder", Value::array({Value(2.0), Value(1.0), Value(3.0)})}})}, {"expected", Value::array({Value(1.0), Value(2.0), Value(3.0)})}, {"visible", Value(false)}})
    };
    std::vector<std::string> argKeys = {"preorder", "inorder"};
    bool failed = false;
    ...
    for (size_t i = 0; i < tests.size(); ++i) {
        ...
        auto result = sol.buildTree(arg0, arg1);
        gotValue = treeToValue(result);
