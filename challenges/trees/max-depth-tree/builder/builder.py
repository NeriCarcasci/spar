...
TESTS = json.loads('[{"input":{"root":[3,9,20,null,null,15,7]},"expected":3,"visible":true},{"input":{"root":[1,null,2]},"expected":2,"visible":true},{"input":{"root":[]},"expected":0,"visible":false},{"input":{"root":[1]},"expected":1,"visible":false},{"input":{"root":[1,2,3,4,5]},"expected":3,"visible":false}]')
COMPARE_MODE = 'exact'
...
def run(solution_path, tests_path, challenge_path):
    ...
    for idx, case in enumerate(TESTS, start=1):
        ...
        fn = resolve_callable(module, ['max_depth'])
        ...
        for key in ['root']:
            ...
            args.append(value)
        out = fn(*args)
        if 'trees/max-depth-tree' in ("trees/build-tree-preorder", "trees/invert-binary-tree"):
