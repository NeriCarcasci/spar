...
TESTS = json.loads('[{"input":{"root":[3,1,4,null,2],"k":1},"expected":1,"visible":true},{"input":{"root":[5,3,6,2,4,null,null,1],"k":3},"expected":3,"visible":true},{"input":{"root":[1],"k":1},"expected":1,"visible":false},{"input":{"root":[2,1,3],"k":2},"expected":2,"visible":false},{"input":{"root":[2,1,3],"k":3},"expected":3,"visible":false}]')
COMPARE_MODE = 'exact'
...
def run(solution_path, tests_path, challenge_path):
    ...
    for idx, case in enumerate(TESTS, start=1):
        ...
        fn = resolve_callable(module, ['kth_smallest'])
        ...
        for key in ['root', 'k']:
            ...
            args.append(value)
        out = fn(*args)
        if 'trees/kth-smallest-bst' in ("trees/build-tree-preorder", "trees/invert-binary-tree"):
