...
TESTS = json.loads('[{"input":{"root":[4,2,7,1,3,6,9]},"expected":[4,7,2,9,6,3,1],"visible":true},{"input":{"root":[2,1,3]},"expected":[2,3,1],"visible":true},{"input":{"root":[]},"expected":[],"visible":false},{"input":{"root":[1]},"expected":[1],"visible":false},{"input":{"root":[1,2,null]},"expected":[1,null,2],"visible":false}]')
COMPARE_MODE = 'exact'
...
def run(solution_path, tests_path, challenge_path):
    ...
    for idx, case in enumerate(TESTS, start=1):
        ...
        fn = resolve_callable(module, ['invert_tree'])
        ...
        for key in ['root']:
            ...
            args.append(value)
        out = fn(*args)
        if 'trees/invert-binary-tree' in ("trees/build-tree-preorder", "trees/invert-binary-tree"):
