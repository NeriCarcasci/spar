...
TESTS = json.loads('[{"input":{"root":[3,9,20,null,null,15,7]},"expected":[[3],[9,20],[15,7]],"visible":true},{"input":{"root":[1]},"expected":[[1]],"visible":true},{"input":{"root":[]},"expected":[],"visible":false},{"input":{"root":[1,2,3,4,5]},"expected":[[1],[2,3],[4,5]],"visible":false},{"input":{"root":[1,null,2,null,3]},"expected":[[1],[2],[3]],"visible":false}]')
COMPARE_MODE = 'exact'
...
def run(solution_path, tests_path, challenge_path):
    ...
    for idx, case in enumerate(TESTS, start=1):
        ...
        fn = resolve_callable(module, ['level_order'])
        ...
        for key in ['root']:
            ...
            args.append(value)
        out = fn(*args)
        if 'trees/level-order-traversal' in ("trees/build-tree-preorder", "trees/invert-binary-tree"):
