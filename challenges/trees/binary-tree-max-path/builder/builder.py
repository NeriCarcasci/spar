...
TESTS = json.loads('[{"input":{"root":[1,2,3]},"expected":6,"visible":true},{"input":{"root":[-10,9,20,null,null,15,7]},"expected":42,"visible":true},{"input":{"root":[1]},"expected":1,"visible":false},{"input":{"root":[-3]},"expected":-3,"visible":false},{"input":{"root":[2,-1,-2]},"expected":2,"visible":false}]')
COMPARE_MODE = 'exact'
...
def run(solution_path, tests_path, challenge_path):
    ...
    for idx, case in enumerate(TESTS, start=1):
        ...
        fn = resolve_callable(module, ['max_path_sum'])
        ...
        for key in ['root']:
            ...
            args.append(value)
        out = fn(*args)
        if 'trees/binary-tree-max-path' in ("trees/build-tree-preorder", "trees/invert-binary-tree"):
