...
TESTS = json.loads('[{"input":{"p":[1,2,3],"q":[1,2,3]},"expected":true,"visible":true},{"input":{"p":[1,2],"q":[1,null,2]},"expected":false,"visible":true},{"input":{"p":[],"q":[]},"expected":true,"visible":false},{"input":{"p":[1],"q":[1]},"expected":true,"visible":false},{"input":{"p":[1,2],"q":[1,2,3]},"expected":false,"visible":false}]')
COMPARE_MODE = 'exact'
...
def run(solution_path, tests_path, challenge_path):
    ...
    for idx, case in enumerate(TESTS, start=1):
        ...
        fn = resolve_callable(module, ['is_same_tree'])
        ...
        for key in ['p', 'q']:
            ...
            args.append(value)
        out = fn(*args)
        if 'trees/same-tree' in ("trees/build-tree-preorder", "trees/invert-binary-tree"):
