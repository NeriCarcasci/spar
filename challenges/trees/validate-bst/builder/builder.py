...
TESTS = json.loads('[{"input":{"root":[2,1,3]},"expected":true,"visible":true},{"input":{"root":[5,1,4,null,null,3,6]},"expected":false,"visible":true},{"input":{"root":[1]},"expected":true,"visible":false},{"input":{"root":[5,4,6,null,null,3,7]},"expected":false,"visible":false},{"input":{"root":[2,2,2]},"expected":false,"visible":false}]')
COMPARE_MODE = 'exact'
...
def run(solution_path, tests_path, challenge_path):
    ...
    for idx, case in enumerate(TESTS, start=1):
        ...
        fn = resolve_callable(module, ['is_valid_bst'])
        ...
        for key in ['root']:
            ...
            args.append(value)
        out = fn(*args)
        if 'trees/validate-bst' in ("trees/build-tree-preorder", "trees/invert-binary-tree"):
