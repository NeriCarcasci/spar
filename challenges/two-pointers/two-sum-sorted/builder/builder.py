...
def run(solution_path, tests_path, challenge_path):
    ...
    for idx, case in enumerate(TESTS, start=1):
        ...
        fn = resolve_callable(module, ['two_sum_sorted'])
        case_input = case.get("input")
        args = []
        if isinstance(case_input, dict):
            for key in ['numbers', 'target']:
                args.append(case_input.get(key))
        elif isinstance(case_input, list) and len(['numbers', 'target']) > 1:
            args.append(case_input)
            for idx in range(1, len(['numbers', 'target'])):
                key = ['numbers', 'target'][idx]
                if key in case:
