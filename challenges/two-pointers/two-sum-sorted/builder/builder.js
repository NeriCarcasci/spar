...
const TESTS = JSON.parse("[{\"input\":{\"numbers\":[2,7,11,15],\"target\":9},\"expected\":[1,2],\"visible\":true},{\"input\":{\"numbers\":[2,3,4],\"target\":6},\"expected\":[1,3],\"visible\":true},{\"input\":{\"numbers\":[-1,0],\"target\":-1},\"expected\":[1,2],\"visible\":false},{\"input\":{\"numbers\":[1,2,3,4,5],\"target\":9},\"expected\":[4,5],\"visible\":false},{\"input\":{\"numbers\":[-3,-1,0,1,5,8],\"target\":4},\"expected\":[2,6],\"visible\":false},{\"input\":{\"numbers\":[1,1,1,1,4],\"target\":5},\"expected\":[1,5],\"visible\":false},{\"input\":{\"numbers\":[-1000,-1,0,1,1000],\"target\":0},\"expected\":[1,5],\"visible\":false}]");
const COMPARE_MODE = "pair_target_1idx";
...
function run(solutionPath, testsPath, challengePath) {
    ...
    const fn = resolveFunction(solution, ["twoSumSorted"]);
    const caseInput = test.input;
    let args = [];
    if (isObject(caseInput)) {
        args = ["numbers", "target"].map((key) => caseInput[key]);
    } else if (Array.isArray(caseInput) && ["numbers", "target"].length > 1) {
        args.push(caseInput);
        for (let i = 1; i < ["numbers", "target"].length; i += 1) {
            const key = ["numbers", "target"][i];
        ...
        args = ["numbers", "target"].map((key) => test[key]);
    }
    gotValue = fn(...args);
