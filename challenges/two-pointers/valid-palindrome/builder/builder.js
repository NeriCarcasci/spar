...
const TESTS = JSON.parse("[{\"input\":{\"s\":\"A man, a plan, a canal: Panama\"},\"expected\":true,\"visible\":true},{\"input\":{\"s\":\"race a car\"},\"expected\":false,\"visible\":true},{\"input\":{\"s\":\" \"},\"expected\":true,\"visible\":false},{\"input\":{\"s\":\"a\"},\"expected\":true,\"visible\":false},{\"input\":{\"s\":\",.\"},\"expected\":true,\"visible\":false},{\"input\":{\"s\":\"0P\"},\"expected\":false,\"visible\":false},{\"input\":{\"s\":\"abba\"},\"expected\":true,\"visible\":false}]");
...
function run(solutionPath, testsPath, challengePath) {
    ...
    const fn = resolveFunction(solution, ["isPalindrome"]);
    const caseInput = test.input;
    let args = [];
    if (isObject(caseInput)) {
        args = ["s"].map((key) => caseInput[key]);
    } else if (Array.isArray(caseInput) && ["s"].length > 1) {
        args.push(caseInput);
