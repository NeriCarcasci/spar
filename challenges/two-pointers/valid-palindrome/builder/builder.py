...
TESTS = json.loads('[{"input":{"s":"A man, a plan, a canal: Panama"},"expected":true,"visible":true},{"input":{"s":"race a car"},"expected":false,"visible":true},{"input":{"s":" "},"expected":true,"visible":false},{"input":{"s":"a"},"expected":true,"visible":false},{"input":{"s":",."},"expected":true,"visible":false},{"input":{"s":"0P"},"expected":false,"visible":false},{"input":{"s":"abba"},"expected":true,"visible":false}]')
COMPARE_MODE = 'exact'
...
def run(solution_path, tests_path, challenge_path):
    ...
    for idx, case in enumerate(TESTS, start=1):
        ...
        fn = resolve_callable(module, ['is_palindrome'])
        case_input = case.get("input")
        args = []
        if isinstance(case_input, dict):
            for key in ['s']:
                args.append(case_input.get(key))
        elif isinstance(case_input, list) and len(['s']) > 1:
