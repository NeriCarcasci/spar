...
int main(int argc, char** argv) {
    ...
    std::vector<Value> tests = {
        Value::object({{"input", Value::object({{"s", Value("abcabcbb")}})}, {"expected", Value(3.0)}, {"visible", Value(true)}}),
        Value::object({{"input", Value::object({{"s", Value("bbbbb")}})}, {"expected", Value(1.0)}, {"visible", Value(true)}}),
        Value::object({{"input", Value::object({{"s", Value("")}})}, {"expected", Value(0.0)}, {"visible", Value(false)}}),
        Value::object({{"input", Value::object({{"s", Value("a")}})}, {"expected", Value(1.0)}, {"visible", Value(false)}}),
        Value::object({{"input", Value::object({{"s", Value("pwwkew")}})}, {"expected", Value(3.0)}, {"visible", Value(false)}}),
        Value::object({{"input", Value::object({{"s", Value("abcdefg")}})}, {"expected", Value(7.0)}, {"visible", Value(false)}}),
        Value::object({{"input", Value::object({{"s", Value("aab")}})}, {"expected", Value(2.0)}, {"visible", Value(false)}})
    };
    std::vector<std::string> argKeys = {"s"};
    bool failed = false;
    ...
    for (size_t i = 0; i < tests.size(); ++i) {
        ...
        auto result = sol.lengthOfLongestSubstring(arg0);
