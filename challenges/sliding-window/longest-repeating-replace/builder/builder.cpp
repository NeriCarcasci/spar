...
int main(int argc, char** argv) {
    ...
    std::vector<Value> tests = {
        Value::object({{"input", Value::object({{"s", Value("ABAB")}, {"k", Value(2.0)}})}, {"expected", Value(4.0)}, {"visible", Value(true)}}),
        Value::object({{"input", Value::object({{"s", Value("AABABBA")}, {"k", Value(1.0)}})}, {"expected", Value(4.0)}, {"visible", Value(true)}}),
        Value::object({{"input", Value::object({{"s", Value("A")}, {"k", Value(0.0)}})}, {"expected", Value(1.0)}, {"visible", Value(false)}}),
        Value::object({{"input", Value::object({{"s", Value("AAAA")}, {"k", Value(0.0)}})}, {"expected", Value(4.0)}, {"visible", Value(false)}}),
        Value::object({{"input", Value::object({{"s", Value("ABCD")}, {"k", Value(0.0)}})}, {"expected", Value(1.0)}, {"visible", Value(false)}}),
        Value::object({{"input", Value::object({{"s", Value("ABCD")}, {"k", Value(3.0)}})}, {"expected", Value(4.0)}, {"visible", Value(false)}}),
        Value::object({{"input", Value::object({{"s", Value("AABBBCCC")}, {"k", Value(2.0)}})}, {"expected", Value(5.0)}, {"visible", Value(false)}})
    };
    std::vector<std::string> argKeys = {"s", "k"};
    bool failed = false;
    ...
    for (size_t i = 0; i < tests.size(); ++i) {
        ...
        auto arg0 = toStringValue(raw0);
        Value raw1 = getArg(testObj, argKeys, 1);
        auto arg1 = toInt(raw1);
        auto result = sol.characterReplacement(arg0, arg1);
