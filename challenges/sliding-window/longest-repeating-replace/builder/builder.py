...
TESTS = json.loads('[{"input":{"s":"ABAB","k":2},"expected":4,"visible":true},{"input":{"s":"AABABBA","k":1},"expected":4,"visible":true},{"input":{"s":"A","k":0},"expected":1,"visible":false},{"input":{"s":"AAAA","k":0},"expected":4,"visible":false},{"input":{"s":"ABCD","k":0},"expected":1,"visible":false},{"input":{"s":"ABCD","k":3},"expected":4,"visible":false},{"input":{"s":"AABBBCCC","k":2},"expected":5,"visible":false}]')
COMPARE_MODE = 'exact'
...
def run(solution_path, tests_path, challenge_path):
    ...
    for idx, case in enumerate(TESTS, start=1):
        ...
        fn = resolve_callable(module, ['character_replacement'])
        case_input = case.get("input")
        args = []
        if isinstance(case_input, dict):
            for key in ['s', 'k']:
                args.append(case_input.get(key))
        elif isinstance(case_input, list) and len(['s', 'k']) > 1:
