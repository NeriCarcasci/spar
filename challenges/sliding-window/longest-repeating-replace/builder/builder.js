...
const TESTS = JSON.parse("[{\"input\":{\"s\":\"ABAB\",\"k\":2},\"expected\":4,\"visible\":true},{\"input\":{\"s\":\"AABABBA\",\"k\":1},\"expected\":4,\"visible\":true},{\"input\":{\"s\":\"A\",\"k\":0},\"expected\":1,\"visible\":false},{\"input\":{\"s\":\"AAAA\",\"k\":0},\"expected\":4,\"visible\":false},{\"input\":{\"s\":\"ABCD\",\"k\":0},\"expected\":1,\"visible\":false},{\"input\":{\"s\":\"ABCD\",\"k\":3},\"expected\":4,\"visible\":false},{\"input\":{\"s\":\"AABBBCCC\",\"k\":2},\"expected\":5,\"visible\":false}]");
...
function run(solutionPath, testsPath, challengePath) {
    ...
    const fn = resolveFunction(solution, ["characterReplacement"]);
    const caseInput = test.input;
    let args = [];
    if (isObject(caseInput)) {
        args = ["s", "k"].map((key) => caseInput[key]);
    } else if (Array.isArray(caseInput) && ["s", "k"].length > 1) {
        args.push(caseInput);
        for (let i = 1; i < ["s", "k"].length; i += 1) {
            const key = ["s", "k"][i];
