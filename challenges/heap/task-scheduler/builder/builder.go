...
func main() {
    ...
    argKeys := []string{"tasks", "n"}
    _ = argKeys
    failed := false
    for i, test := range tests {
        var gotValue any
        raw0 := getArg(test, argKeys, 0)
        arg0 := toByteSlice(raw0)
        raw1 := getArg(test, argKeys, 1)
        arg1 := toInt(raw1)
        result := LeastInterval(arg0, arg1)
