...
TESTS = json.loads('[{"input":{"s":"anagram","t":"nagaram"},"expected":true,"visible":true},{"input":{"s":"rat","t":"car"},"expected":false,"visible":true},{"input":{"s":"a","t":"a"},"expected":true,"visible":false},{"input":{"s":"ab","t":"a"},"expected":false,"visible":false},{"input":{"s":"","t":""},"expected":true,"visible":false},{"input":{"s":"aabbcc","t":"abcabc"},"expected":true,"visible":false},{"input":{"s":"aacc","t":"ccac"},"expected":false,"visible":false}]')
COMPARE_MODE = 'exact'
...
def run(solution_path, tests_path, challenge_path):
    ...
    for idx, case in enumerate(TESTS, start=1):
        ...
        fn = resolve_callable(module, ['is_anagram'])
        case_input = case.get("input")
        args = []
        if isinstance(case_input, dict):
            for key in ['s', 't']:
                args.append(case_input.get(key))
        elif isinstance(case_input, list) and len(['s', 't']) > 1:
            args.append(case_input)
            for idx in range(1, len(['s', 't'])):
                key = ['s', 't'][idx]
                if key in case:
