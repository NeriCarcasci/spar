...
const TESTS = JSON.parse("[{\"input\":{\"temperatures\":[73,74,75,71,69,72,76,73]},\"expected\":[1,1,4,2,1,1,0,0],\"visible\":true},{\"input\":{\"temperatures\":[30,40,50,60]},\"expected\":[1,1,1,0],\"visible\":true},{\"input\":{\"temperatures\":[50]},\"expected\":[0],\"visible\":false},{\"input\":{\"temperatures\":[60,50,40,30]},\"expected\":[0,0,0,0],\"visible\":false},{\"input\":{\"temperatures\":[30,30,30]},\"expected\":[0,0,0],\"visible\":false},{\"input\":{\"temperatures\":[70,60,80]},\"expected\":[2,1,0],\"visible\":false},{\"input\":{\"temperatures\":[30,100,30,100]},\"expected\":[1,0,1,0],\"visible\":false}]");
...
function run(solutionPath, testsPath, challengePath) {
    ...
    const fn = resolveFunction(solution, ["dailyTemperatures"]);
    const caseInput = test.input;
    let args = [];
    if (isObject(caseInput)) {
        args = ["temperatures"].map((key) => caseInput[key]);
    } else if (Array.isArray(caseInput) && ["temperatures"].length > 1) {
        args.push(caseInput);
        for (let i = 1; i < ["temperatures"].length; i += 1) {
            const key = ["temperatures"][i];
        ...
        args = ["temperatures"].map((key) => test[key]);
    }
    gotValue = fn(...args);
