...
const TESTS = JSON.parse("[{\"input\":{\"numCourses\":2,\"prerequisites\":[[1,0]]},\"expected\":true,\"visible\":true},{\"input\":{\"numCourses\":2,\"prerequisites\":[[1,0],[0,1]]},\"expected\":false,\"visible\":true},{\"input\":{\"numCourses\":1,\"prerequisites\":[]},\"expected\":true,\"visible\":false},{\"input\":{\"numCourses\":5,\"prerequisites\":[[1,0],[2,1],[3,2],[4,3]]},\"expected\":true,\"visible\":false},{\"input\":{\"numCourses\":4,\"prerequisites\":[[1,0],[2,0],[3,1],[3,2]]},\"expected\":true,\"visible\":false},{\"input\":{\"numCourses\":3,\"prerequisites\":[[0,1],[1,2],[2,0]]},\"expected\":false,\"visible\":false},{\"input\":{\"numCourses\":5,\"prerequisites\":[]},\"expected\":true,\"visible\":false}]");
...
function run(solutionPath, testsPath, challengePath) {
    ...
    const fn = resolveFunction(solution, ["canFinish"]);
    const caseInput = test.input;
    let args = [];
    if (isObject(caseInput)) {
        args = ["numCourses", "prerequisites"].map((key) => caseInput[key]);
    } else if (Array.isArray(caseInput) && ["numCourses", "prerequisites"].length > 1) {
        args.push(caseInput);
        for (let i = 1; i < ["numCourses", "prerequisites"].length; i += 1) {
            const key = ["numCourses", "prerequisites"][i];
        ...
        args = ["numCourses", "prerequisites"].map((key) => test[key]);
    }
    gotValue = fn(...args);
