...
TESTS = json.loads('[{"input":{"numCourses":2,"prerequisites":[[1,0]]},"expected":true,"visible":true},{"input":{"numCourses":2,"prerequisites":[[1,0],[0,1]]},"expected":false,"visible":true},{"input":{"numCourses":1,"prerequisites":[]},"expected":true,"visible":false},{"input":{"numCourses":5,"prerequisites":[[1,0],[2,1],[3,2],[4,3]]},"expected":true,"visible":false},{"input":{"numCourses":4,"prerequisites":[[1,0],[2,0],[3,1],[3,2]]},"expected":true,"visible":false},{"input":{"numCourses":3,"prerequisites":[[0,1],[1,2],[2,0]]},"expected":false,"visible":false},{"input":{"numCourses":5,"prerequisites":[]},"expected":true,"visible":false}]')
COMPARE_MODE = 'exact'
...
def run(solution_path, tests_path, challenge_path):
    ...
    for idx, case in enumerate(TESTS, start=1):
        ...
        fn = resolve_callable(module, ['can_finish'])
        case_input = case.get("input")
        args = []
        if isinstance(case_input, dict):
            for key in ['numCourses', 'prerequisites']:
                args.append(case_input.get(key))
        elif isinstance(case_input, list) and len(['numCourses', 'prerequisites']) > 1:
            args.append(case_input)
            for idx in range(1, len(['numCourses', 'prerequisites'])):
                key = ['numCourses', 'prerequisites'][idx]
                if key in case:
