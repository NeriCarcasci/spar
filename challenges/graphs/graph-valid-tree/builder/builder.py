...
TESTS = json.loads('[{"input":{"n":5,"edges":[[0,1],[0,2],[0,3],[1,4]]},"expected":true,"visible":true},{"input":{"n":5,"edges":[[0,1],[1,2],[2,3],[1,3],[1,4]]},"expected":false,"visible":true},{"input":{"n":1,"edges":[]},"expected":true,"visible":false},{"input":{"n":4,"edges":[[0,1],[2,3]]},"expected":false,"visible":false},{"input":{"n":2,"edges":[[0,1]]},"expected":true,"visible":false},{"input":{"n":3,"edges":[[0,1],[1,2],[0,2]]},"expected":false,"visible":false}]')
COMPARE_MODE = 'exact'
...
def run(solution_path, tests_path, challenge_path):
    ...
    for idx, case in enumerate(TESTS, start=1):
        ...
        fn = resolve_callable(module, ['valid_tree'])
        case_input = case.get("input")
        args = []
        if isinstance(case_input, dict):
            for key in ['n', 'edges']:
                args.append(case_input.get(key))
        elif isinstance(case_input, list) and len(['n', 'edges']) > 1:
            args.append(case_input)
            for idx in range(1, len(['n', 'edges'])):
                key = ['n', 'edges'][idx]
                if key in case:
