...
TESTS = json.loads('[{"input":{"heights":[[1,2,2,3,5],[3,2,3,4,4],[2,4,5,3,1],[6,7,1,4,5],[5,1,1,2,4]]},"expected":[[0,4],[1,3],[1,4],[2,2],[3,0],[3,1],[4,0]],"visible":true},{"input":{"heights":[[1]]},"expected":[[0,0]],"visible":true},{"input":{"heights":[[1,2,3],[8,9,4],[7,6,5]]},"expected":[[0,2],[1,0],[1,1],[1,2],[2,0],[2,1],[2,2]],"visible":false},{"input":{"heights":[[10,10,10],[10,1,10],[10,10,10]]},"expected":[[0,0],[0,1],[0,2],[1,0],[1,2],[2,0],[2,1],[2,2]],"visible":false},{"input":{"heights":[[1,1],[1,1],[1,1]]},"expected":[[0,0],[0,1],[1,0],[1,1],[2,0],[2,1]],"visible":false}]')
...
def run(solution_path, tests_path, challenge_path):
    ...
    for idx, case in enumerate(TESTS, start=1):
        ...
        fn = resolve_callable(module, ['pacific_atlantic'])
        case_input = case.get("input")
        args = []
        if isinstance(case_input, dict):
            for key in ['heights']:
                args.append(case_input.get(key))
        elif isinstance(case_input, list) and len(['heights']) > 1:
