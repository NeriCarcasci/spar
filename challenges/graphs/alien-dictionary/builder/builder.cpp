...
int main(int argc, char** argv) {
    ...
    std::vector<Value> tests = {
        Value::object({{"input", Value::object({{"words", Value::array({Value("wrt"), Value("wrf"), Value("er"), Value("ett"), Value("rftt")})}})}, {"expected", Value("wertf")}, {"visible", Value(true)}}),
        Value::object({{"input", Value::object({{"words", Value::array({Value("z"), Value("x")})}})}, {"expected", Value("zx")}, {"visible", Value(true)}}),
        Value::object({{"input", Value::object({{"words", Value::array({Value("z"), Value("x"), Value("z")})}})}, {"expected", Value("")}, {"visible", Value(false)}}),
        Value::object({{"input", Value::object({{"words", Value::array({Value("abc"), Value("ab")})}})}, {"expected", Value("")}, {"visible", Value(false)}}),
        Value::object({{"input", Value::object({{"words", Value::array({Value("z")})}})}, {"expected", Value("z")}, {"visible", Value(false)}}),
        Value::object({{"input", Value::object({{"words", Value::array({Value("a"), Value("b"), Value("ca"), Value("cc")})}})}, {"expected", Value("abc")}, {"visible", Value(false)}}),
        Value::object({{"input", Value::object({{"words", Value::array({Value("ab"), Value("abc")})}})}, {"expected", Value("abc")}, {"visible", Value(false)}})
    };
    std::vector<std::string> argKeys = {"words"};
    bool failed = false;
    ...
    for (size_t i = 0; i < tests.size(); ++i) {
        ...
        auto result = sol.alienOrder(arg0);
