...
function run(solutionPath, testsPath, challengePath) {
    ...
    const fn = resolveFunction(solution, ["combinationSum"]);
    const caseInput = test.input;
    let args = [];
    if (isObject(caseInput)) {
        args = ["candidates", "target"].map((key) => caseInput[key]);
    } else if (Array.isArray(caseInput) && ["candidates", "target"].length > 1) {
        args.push(caseInput);
        for (let i = 1; i < ["candidates", "target"].length; i += 1) {
            const key = ["candidates", "target"][i];
        ...
        args = ["candidates", "target"].map((key) => test[key]);
    }
    gotValue = fn(...args);
