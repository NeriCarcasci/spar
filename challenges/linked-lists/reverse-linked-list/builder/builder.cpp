...
int main(int argc, char** argv) {
    ...
    std::vector<Value> tests = {
        Value::object({{"input", Value::array({Value(1.0), Value(2.0), Value(3.0), Value(4.0), Value(5.0)})}, {"expected", Value::array({Value(5.0), Value(4.0), Value(3.0), Value(2.0), Value(1.0)})}}),
        Value::object({{"input", Value::array({Value(1.0), Value(2.0)})}, {"expected", Value::array({Value(2.0), Value(1.0)})}}),
        Value::object({{"input", Value::array({})}, {"expected", Value::array({})}}),
        Value::object({{"input", Value::array({Value(1.0)})}, {"expected", Value::array({Value(1.0)})}}),
        Value::object({{"input", Value::array({Value(1.0), Value(2.0), Value(3.0)})}, {"expected", Value::array({Value(3.0), Value(2.0), Value(1.0)})}}),
        Value::object({{"input", Value::array({Value(5.0), Value(4.0), Value(3.0), Value(2.0), Value(1.0)})}, {"expected", Value::array({Value(1.0), Value(2.0), Value(3.0), Value(4.0), Value(5.0)})}})
    };
    std::vector<std::string> argKeys = {"head"};
    bool failed = false;
    ...
    for (size_t i = 0; i < tests.size(); ++i) {
        ...
        Solution sol;
        gotValue = listToValue(sol.reverseList(buildList(getField(testObj, "input"))));
