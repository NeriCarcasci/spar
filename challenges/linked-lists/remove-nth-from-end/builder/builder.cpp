...
int main(int argc, char** argv) {
    ...
    std::vector<Value> tests = {
        Value::object({{"input", Value::object({{"list", Value::array({Value(1.0), Value(2.0), Value(3.0), Value(4.0), Value(5.0)})}, {"n", Value(2.0)}})}, {"expected", Value::array({Value(1.0), Value(2.0), Value(3.0), Value(5.0)})}}),
        Value::object({{"input", Value::object({{"list", Value::array({Value(1.0)})}, {"n", Value(1.0)}})}, {"expected", Value::array({})}}),
        Value::object({{"input", Value::object({{"list", Value::array({Value(1.0), Value(2.0)})}, {"n", Value(1.0)}})}, {"expected", Value::array({Value(1.0)})}}),
        Value::object({{"input", Value::object({{"list", Value::array({Value(1.0), Value(2.0)})}, {"n", Value(2.0)}})}, {"expected", Value::array({Value(2.0)})}}),
        Value::object({{"input", Value::object({{"list", Value::array({Value(1.0), Value(2.0), Value(3.0)})}, {"n", Value(3.0)}})}, {"expected", Value::array({Value(2.0), Value(3.0)})}}),
        Value::object({{"input", Value::object({{"list", Value::array({Value(1.0), Value(2.0), Value(3.0), Value(4.0), Value(5.0)})}, {"n", Value(5.0)}})}, {"expected", Value::array({Value(2.0), Value(3.0), Value(4.0), Value(5.0)})}})
    };
    std::vector<std::string> argKeys = {"list", "n"};
    bool failed = false;
    ...
    for (size_t i = 0; i < tests.size(); ++i) {
        ...
        gotValue = listToValue(sol.removeNthFromEnd(buildList(getField(input, "list")), toInt(getField(input, "n"))));
