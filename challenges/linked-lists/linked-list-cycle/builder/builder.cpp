...
int main(int argc, char** argv) {
    ...
    std::vector<Value> tests = {
        Value::object({{"input", Value::object({{"list", Value::array({Value(3.0), Value(2.0), Value(0.0), Value(-4.0)})}, {"pos", Value(1.0)}})}, {"expected", Value(true)}}),
        Value::object({{"input", Value::object({{"list", Value::array({Value(1.0), Value(2.0)})}, {"pos", Value(0.0)}})}, {"expected", Value(true)}}),
        Value::object({{"input", Value::object({{"list", Value::array({Value(1.0)})}, {"pos", Value(-1.0)}})}, {"expected", Value(false)}}),
        Value::object({{"input", Value::object({{"list", Value::array({})}, {"pos", Value(-1.0)}})}, {"expected", Value(false)}}),
        Value::object({{"input", Value::object({{"list", Value::array({Value(1.0), Value(2.0), Value(3.0), Value(4.0)})}, {"pos", Value(-1.0)}})}, {"expected", Value(false)}}),
        Value::object({{"input", Value::object({{"list", Value::array({Value(1.0), Value(2.0), Value(3.0), Value(4.0)})}, {"pos", Value(2.0)}})}, {"expected", Value(true)}})
    };
    std::vector<std::string> argKeys = {"list", "pos"};
    bool failed = false;
    ...
    for (size_t i = 0; i < tests.size(); ++i) {
        ...
        gotValue = toValue(sol.hasCycle(head));
