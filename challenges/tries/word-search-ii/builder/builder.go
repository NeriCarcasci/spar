...
func main() {
    ...
    argKeys := []string{"board", "words"}
    _ = argKeys
    failed := false
    for i, test := range tests {
        var gotValue any
        raw0 := getArg(test, argKeys, 0)
        arg0 := toByteMatrix(raw0)
        raw1 := getArg(test, argKeys, 1)
        arg1 := toStringSlice(raw1)
        result := FindWords(arg0, arg1)
